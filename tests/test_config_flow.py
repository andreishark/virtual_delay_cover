"""Test the virtual_delay_cover config flow."""
...
from homeassistant.components.virtual_delay_cover.const import DOMAIN
...
async def test_config_flow(
    ...
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"],
        {"name": "My virtual_delay_cover", "entity_id": input_sensor_entity_id},
    )
    await hass.async_block_till_done()

    assert result["type"] is FlowResultType.CREATE_ENTRY
    assert result["title"] == "My virtual_delay_cover"
    assert result["data"] == {}
    assert result["options"] == {
        "entity_id": input_sensor_entity_id,
        "name": "My virtual_delay_cover",
    }
    ...
    assert config_entry.title == "My virtual_delay_cover"
...
async def test_options(hass: HomeAssistant, platform) -> None:
    ...
    state = hass.states.get(f"{platform}.my_virtual_delay_cover")
